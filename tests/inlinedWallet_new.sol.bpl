...
// #LTLProperty: []((AP(withdrawCalled == true) && AP(withdrawAmt <= userBal)) ==> <>(AP(fallbackSucceeded == true) && AP(fallbackAmt == withdrawAmt)))
//  <>[]AP(__now==0)
type address_t = int;
    ...
    modifies __balance, balance#5, __now, userBal, fallbackSucceeded, fallbackAmt, withdrawCalled, withdrawAddr, withdrawAmt, fallbackAddr;
{
    ...
    while (*) {
        ...
        //assume(balance#5[contract][address] <= __balance[contract]);

        //call userBal := getBalance(contract, user, 0);
        assume(__balance[address] > 0);
        userBal := balance#5[contract][address];
