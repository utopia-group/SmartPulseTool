...
procedure {:sourceloc "simpleAuction.sol", 15, 5} {:message "SimpleAuction::bid"} bid#49(__this: address_t, __msg_sender: address_t, __msg_value: int, loc_ended: [address_t]bool) returns (ret: [address_t]bool)
    ...
    modifies owner#3, highestBidder#5, highestBid#7, ended#9, __balance, fallbackAmt, fallbackAddr, fallbackSucceeded, bidAddr, bidAmt, bidCalled, ended;
{
    ...
    while(*) {
        ...
        if(*) {
            call ended#9 := bid#49(contract, addr, amt, ended#9);
        }
        else {
