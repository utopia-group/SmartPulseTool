...
cp template.mak ${CONTRACT_DIR}/Makefile
cp template.ini ${CONTRACT_DIR}/${CONTRACT_NAME%.sol}-erc20-spec.ini
...
solc --bin --overwrite -o ${CONTRACT_DIR}/solc_out/ ${CONTRACT_DIR}/${CONTRACT_NAME} > /dev/null
printf "code: \"0x" >> ${CONTRACT_DIR}/${CONTRACT_NAME%.sol}-erc20-spec.ini
cat ${CONTRACT_DIR}/solc_out/${CONTRACT_SHORT_NAME}.bin >> ${CONTRACT_DIR}/${CONTRACT_NAME%.sol}-erc20-spec.ini
