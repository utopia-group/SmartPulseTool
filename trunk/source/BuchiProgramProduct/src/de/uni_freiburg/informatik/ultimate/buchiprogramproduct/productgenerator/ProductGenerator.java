...
public final class ProductGenerator {
	...
	private static final boolean USE_SC_ACCEPT_LOCS = true;
	...
	public ProductGenerator(final INestedWordAutomaton<CodeBlock, String> nwa, final BoogieIcfgContainer rcfg,
		...
		if(USE_SC_ACCEPT_LOCS) {
			TransFormulaAdder tfa = new TransFormulaAdder(rcfg.getBoogie2SMT(), services);
			for(CodeBlock blk : nwa.getAlphabet()) {
				tfa.addTransitionFormulas(blk, "main", xnfConversionTechnique, simplificationTechnique);
			}
		}
	...
	public boolean isCfgStateAccepting(String ltlState, BoogieIcfgLocation loc) {
		if(USE_SC_ACCEPT_LOCS) {
			if(loc.getProcedure().equals("main")) {
				return true;
			}
			
			Set<IProgramVar> vars = new HashSet<>();
			for(IncomingInternalTransition<CodeBlock, String> incoming : mNWA.internalPredecessors(ltlState)) {
				CodeBlock blk = incoming.getLetter();
				vars.addAll(blk.getTransformula().getInVars().keySet());
			}
			
			for(IcfgEdge edge : loc.getIncomingEdges()) {
				if((edge.getTransformula() == null) || (edge instanceof Summary && ((Summary) edge).calledProcedureHasImplementation())) {
					continue;
				}
				
				Set<IProgramVar> assigned = edge.getTransformula().getAssignedVars();
				if(vars.stream().anyMatch(v -> assigned.contains(v))) {
					return true;
				}
			}
	...
	private void createProductStates() {
		for (final BoogieIcfgLocation origpp : mRCFGLocations) {
			...
			for (final String nwaState : mNWA.getStates()) {
				...
				if (mNWA.isFinal(nwaState) && isCfgStateAccepting(nwaState, origpp)) {
	...
	private void pruneNonProductSinks() {
		...
		Set<BoogieIcfgLocation> cpy = new HashSet<>(mHelperProductStates);
		
		for (final BoogieIcfgLocation helper : cpy) {
			if(mHelperProductStates.contains(helper)) {
	...
	private void pruneNonProductSink(final BoogieIcfgLocation helper) {
		...
		for (final String nwaState : mNWA.getStates()) {
			for (final IcfgLocation node : predecessors) {
				...
				if (mNWA.isFinal(nwaState) && isCfgStateAccepting(nwaState, helper)) {
		...
		//An additional edge is added if consistent with the LTL formula. If we don't remove seq we will have a self-loop that is inconsistent.
		//if (added) {
	...
	private void generateTransFormulas() {
		...
		for(final Entry<DebugIdentifier, BoogieIcfgLocation> loc : mProductLocations.entrySet()) {
			for(final IcfgEdge edge : loc.getValue().getOutgoingEdges()) {
				generateTransformula(tfb, edge.getPrecedingProcedure(), edge);
			}
		}
	...
	private StatementSequence createNewStatementSequence(final BoogieIcfgLocation currentpp,
		...
		final TransFormulaBuilder tfb =
				new TransFormulaBuilder(mProductRoot, mServices, mSimplificationTechnique, mXnfConversionTechnique);
		generateTransformula(tfb, newSS.getPrecedingProcedure(), newSS);
