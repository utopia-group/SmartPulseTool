...
public class LTL2BAExecutor {

	private final ILogger mLogger;
	private final IUltimateServiceProvider mServices;
	private final IToolchainStorage mStorage;

	public LTL2BAExecutor(final IUltimateServiceProvider services, final IToolchainStorage storage) {
		mServices = services;
		mStorage = storage;
	 ...
	 * @throws Exception
	 */
	public AstNode ltl2Ast(final String ltlFormula, boolean negate) throws Exception {
		final String toolOutput = execLTLXBA(ltlFormula.trim(), negate);
	 ...
	 */
	private String execLTLXBA(final String ltlFormula, boolean negate) throws IOException, InterruptedException {
		final String[] command = getCommand(ltlFormula, negate);
	...
	private String[] getCommand(String ltlFormula, boolean negate) {
		final IPreferenceProvider prefs = mServices.getPreferenceProvider("de.uni_freiburg.informatik.ultimate.ltl2aut");
		if(negate) {
