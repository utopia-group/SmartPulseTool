...
public class NestedInterpolantsBuilder {
	...
	private final Object mScriptLockOwner;
	...
	public NestedInterpolantsBuilder(final ManagedScript mgdScriptTc, final Object scriptLockOwner,
			...
			final XnfConversionTechnique xnfConversionTechnique, int suffixStart) {
		mServices = services;
		...
		mCraigInterpolants = new Term[mAnnotSSA.getTrace().length() - suffixStart - 1];
		...
		computeCraigInterpolants(suffixStart);
	...
	public NestedInterpolantsBuilder(final ManagedScript mgdScriptTc, final Object scriptLockOwner,
		...
		this(mgdScriptTc, scriptLockOwner, annotatdSsa, constants2BoogieVar, predicateBuilder, predicateFactory,
				interpolatedPositions, treeInterpolation, services, mgdScriptCfg, instantiateArrayExt, 
				simplificationTechnique, xnfConversionTechnique, 0);
	...
	public void computeCraigInterpolants() {
		computeCraigInterpolants(0);
	}

	public void computeCraigInterpolants(int suffixStart) {
		...
		for (int i = suffixStart; i < mAnnotSSA.getTrace().length(); i++) {
		...
		if (mTrace.isCallPosition(suffixStart) && !mTrace.isPendingCall(suffixStart)) {
			final int correspondingReturn = mTrace.getReturnPosition(suffixStart);
			// if we do not interpolate at each position
			// interpolInput[correspondingReturn] might be the wrong position
			int interpolInputPositionOfReturn = suffixStart;
			for (int i = suffixStart; i < correspondingReturn; i++) {
	...
	public IPredicate[] getNestedInterpolants() {
		if(mInterpolants == null) {
			assert mCraigInterpolants != null;
