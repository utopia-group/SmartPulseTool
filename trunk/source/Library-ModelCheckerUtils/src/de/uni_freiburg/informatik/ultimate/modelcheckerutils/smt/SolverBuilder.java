...
public class SolverBuilder {
	...
	private static Script createExternalSolver(final IUltimateServiceProvider services, final IToolchainStorage storage,
			...
			final boolean useDiffWrapper, final boolean omitLogicOutput) throws IOException {
		final ILogger solverLogger = services.getLoggingService().getLoggerForExternalTool(SOLVER_LOGGER_NAME);
		Script script;
		if (fakeNonIncrementalScript) {
			...
			script = new Scriptor(command, solverLogger, services, storage, "External", omitLogicOutput);
		}
		if (useDiffWrapper) {
			...
			try {
				if (settings.getExternalInterpolator() == null) {
					result = createExternalSolver(services, storage, settings.getCommandExternalSolver(),
							...
							settings.getUseDiffWrapper(), settings.omitLogicOutput());
				} else {
					solverLogger.info(
	...
	public static SolverSettings constructSolverSettings(final String filename, final SolverMode solverMode,
		...
		return constructSolverSettings(filename, solverMode, fakeNonIncrementalScript, commandExternalSolver, dumpSmtScriptToFile,
				pathOfDumpedScript, false);
	}
	
	public static SolverSettings constructSolverSettings(final String filename, final SolverMode solverMode,
			final boolean fakeNonIncrementalScript, final String commandExternalSolver,
			final boolean dumpSmtScriptToFile, final String pathOfDumpedScript, boolean omitLogicOutput) throws AssertionError {
	...
	public static final class SolverSettings {
		...
		private final boolean mOmitLogicOutput;
		...
		public SolverSettings(final boolean fakeNonIncrementalScript, final boolean useExternalSolver,
			...
			this(fakeNonIncrementalScript, useExternalSolver, commandExternalSolver, timeoutSmtInterpol,
					externalInterpolator, dumpSmtScriptToFile, pathOfDumpedScript, baseNameOfDumpedScript, useDiffWrapper,
					false);
		}
		...
		public SolverSettings(final boolean fakeNonIncrementalScript, final boolean useExternalSolver,
			...
			mOmitLogicOutput = omitLogicOutput;
		}
		
		public boolean omitLogicOutput() {
			return mOmitLogicOutput;
		}
