...
interface MinterInterface {
  function cloneToken(string calldata _uri, address _erc20Address) external returns (address asset);
...
interface Events {
  function transaction(string calldata _message, address _from, address _to, uint _amount, address _token)  external;
  function asset(string calldata _message, string calldata _uri, address _assetAddress, address _manager) external;
}
interface DB {
...
contract CrowdsaleERC20{
  ...
  */
  ...
  function convertTokens(address payable _investor, uint _amount, /*bytes4 _sig,*/ ERC20 _fundingToken, ERC20 _paymentToken, uint _maxTokens)
  private
  returns (uint) {
    ...
    if(address(_paymentToken) == address(0xEeeeeEeeeEeEeeEeEeEeeEEEeeeeEeeeeeeeEEeE)){
      ...
      fundingBalanceBefore = _fundingToken.balanceOf(address(this));
      //Convert remaining funds into the funding token
      kyber.trade.value(_amount)(address(_paymentToken), _amount, address(_fundingToken), address(this), _maxTokens, 0, address(0));
      change = _amount.sub(paymentBalanceBefore.sub(address(this).balance));
      investment = _fundingToken.balanceOf(address(this)).sub(fundingBalanceBefore);
      if(change > 0){
      ...
      paymentBalanceBefore = _paymentToken.balanceOf(address(this));
      fundingBalanceBefore = _fundingToken.balanceOf(address(this));
      //Convert remaining funds into the funding token
      kyber.trade(address(_paymentToken), _amount, address(_fundingToken), address(this), _maxTokens, 0, address(0));
      // Return any remaining source tokens to user
      change = _amount.sub(paymentBalanceBefore.sub(_paymentToken.balanceOf(address(this))));
      investment = _fundingToken.balanceOf(address(this)).sub(fundingBalanceBefore);
      if(change > 0){
