...
contract AccountModifiersInterface {
  function accountModifiers(address _user) public view returns(uint takeFeeDiscount, uint rebatePercentage);
  function tradeModifiers(address _maker, address _taker) public view returns(uint takeFeeDiscount, uint rebatePercentage);
}

// Interface for trade tracker

contract TradeTrackerInterface {
  function tradeComplete(address _tokenGet, uint _amountGet, address _tokenGive, uint _amountGive, address _get, address _give, uint _takerFee, uint _makerRebate) public;
}
...
contract TokenStore is SafeMath, Ownable {
  ...
  function getAccountModifiers() public view returns(uint takeFeeDiscount, uint rebatePercentage) {
  ...
  function depositToken(address _token, uint _amount) public deprecable {
  ...
  function trade(address _tokenGet, uint _amountGet, address _tokenGive, uint _amountGive,
      uint _expires, uint _nonce, address _user, uint8 _v, bytes32 _r, bytes32 _s, uint _amount) public {
    ...
    emit Trade(_tokenGet, _amount, _tokenGive, _amountGive * _amount / _amountGet, _user, msg.sender, _nonce);
  }
  ...
  function testTrade(address _tokenGet, uint _amountGet, address _tokenGive, uint _amountGive, uint _expires,
      uint _nonce, address _user, uint8 _v, bytes32 _r, bytes32 _s, uint _amount, address _sender) public view returns(bool) {
  ...
  function availableVolume(address _tokenGet, uint _amountGet, address _tokenGive, uint _amountGive, uint _expires,
      uint _nonce, address _user, uint8 _v, bytes32 _r, bytes32 _s) public view returns(uint) {
    bytes32 hash = sha256(this, _tokenGet, _amountGet, _tokenGive, _amountGive, _expires, _nonce);
    if (ecrecover(sha3("\x19Ethereum Signed Message:\n32", hash), _v, _r, _s) != _user ||
      block.number > _expires) {
      return 0;
  ...
  function amountFilled(address _tokenGet, uint _amountGet, address _tokenGive, uint _amountGive, uint _expires,
      uint _nonce, address _user) public view returns(uint) {
    bytes32 hash = sha256(this, _tokenGet, _amountGet, _tokenGive, _amountGive, _expires, _nonce);
  ...
  function cancelOrder(address _tokenGet, uint _amountGet, address _tokenGive, uint _amountGive, uint _expires,
      uint _nonce, uint8 _v, bytes32 _r, bytes32 _s) public {
    bytes32 hash = sha256(this, _tokenGet, _amountGet, _tokenGive, _amountGive, _expires, _nonce);
  ...
  function migrateFunds(address[] memory _tokens) public {
    ...
    // Ether
    uint etherAmount = tokens[address(0)][msg.sender];
    if (etherAmount > 0) {
      tokens[address(0)][msg.sender] = 0;
    ...
    for (uint n = 0; n < _tokens.length; n++) {
      ...
      if (!Token(token).approve(address(newExchange), tokenAmount)) {
        revert();
      }
  ...
  function depositForUser(address _user) public payable deprecable {
    require(_user != address(0));
    require(msg.value > 0);
    TokenStore caller = TokenStore(msg.sender);
    require(caller.version() > 0); // Make sure it's an exchange account
    tokens[address(0)][_user] = safeAdd(tokens[address(0)][_user], msg.value);
  }

  function depositTokenForUser(address _token, uint _amount, address _user) public deprecable {
    require(_token != address(0));
    require(_user != address(0));
    require(_amount > 0);
    TokenStore caller = TokenStore(msg.sender);
