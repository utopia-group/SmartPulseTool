...
// Spec: L2
// LTLVariables: val:int,ben:Ref
// LTLFairness: [](<>(call(RefundEscrow.beneficiaryWithdraw) {}))
// LTLProperty: []((success(RefundEscrow.deposit) {val == msg.value} ; success(RefundEscrow.close) {ben == RefundEscrow.beneficiary()}) ==> <>(success(send(from, to, amt)) {to == ben && amt >= val}))
