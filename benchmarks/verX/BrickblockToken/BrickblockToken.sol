/**
 * Source Code first verified at https://etherscan.io on Friday, December 8, 2017
 (UTC) */
...
contract BrickblockToken is PausableToken {
  ...
  uint256 public constant initialSupply = 500 * (10 ** 6) * 1000000000000000000;
  ...
  modifier supplyAvailable(uint256 _value) {
    ...
    require(initialSupply.sub(balances[address(this)]).add(_value) <= initialSupply * 51 / 100);
    _;
  }
    ...
    public
  {
    ...
    if (_predecessorAddress != address(0)) {
      ...
      balances[address(this)] = predecessor.balanceOf(_predecessorAddress);
      emit Transfer(address(0), address(this), predecessor.balanceOf(_predecessorAddress));
    ...
    returns (bool)
  {
    ...
    if (_oldBalance > 0) {
      ...
      emit Rescued(_user, _oldBalance, balances[_user]);
      return true;
    }
    return false;
  }
