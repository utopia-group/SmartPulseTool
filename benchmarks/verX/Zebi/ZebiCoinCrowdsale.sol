/**
 * Source Code first verified at https://etherscan.io on Sunday, March 4, 2018
 (UTC) */
...
contract ZCrowdsale is Ownable{
  ...
  constructor(uint256 _startTime, uint256 _endTime, uint256 _ETHtoZCOrate, address payable _wallet,uint256 _minTransAmount,uint256 _mintedTokensCap) public {
    ...
    mintedTokensCap = _mintedTokensCap * 100000000;            // mintedTokensCap is in Zwei 
  ...
  function getTokenAmount(uint256 weiAmount) public view returns(uint256) {                      
	...
	//uint256 ETHtoZweiRate = ETHtoZCOrate * (100000000);
    //return  SafeMath.div((weiAmount.mul(ETHtoZweiRate)),(1 ether));
    return (weiAmount * 100000000) / 1000000000000000000;
  }
  ...
  function validPurchase() internal view returns (bool) {
   ...
   function refund() external inCancelledList inRefundPeriod {                                                    
       ...
       contribution[msg.sender] = 0;
		...
		emit Refund(msg.sender,refundBalance);
    } 
	
	function forcedRefund(address payable _from) external onlyOwner {
       ...
       contribution[_from] = 0;
		...
		emit Refund(_from,refundBalance);
    ...
    function takeEth() external payable {
		emit TakeEth(msg.sender,msg.value);
    }
...
contract ZebiCoinCrowdsale is ZCrowdsale {
  ...
  constructor(uint256 _startTime, uint256 _endTime, uint256 _rate, address payable _wallet,uint256 _minTransAmount,uint256 _mintedTokensCap) public
...
contract ZebiCoinTempMgr is Ownable{
  ...
  constructor(address payable presaleCrowdsale, address tokenAddress, address payable _wallet) public {
  ...
  function allocatenonCSTokens(address beneficiary,uint256 tokens) external onlyOwner
  {
	...
	//uint256 Zweitokens = tokens.mul(100000000);
    uint256 Zweitokens = tokens * (100000000);
  ...
  function refund() external inPreSaleCancelledList inRefundPeriod2 {                                                    
    ...
    tsc.burn(msg.sender);
    msg.sender.transfer(refundBalance); 
	emit PreSaleRefund(msg.sender,refundBalance);
  } 
  ...
  function forcedRefund(address payable _from) external onlyOwner {
	...
	tsc.burn(_from);
    _from.transfer(refundBalance); 
	emit PreSaleRefund(_from,refundBalance);
  }
...
contract ZebiMainCrowdsale is Ownable{
  ...
  modifier inCancelledList1 {
    require(mainCancelledList[msg.sender]);
    _;
  }
  
  modifier inRefundPeriod1 {
  require(withinRefundPeriod);
  _;
  }  
  ...
  constructor(uint256 _startTime, uint256 _endTime, uint256 _ETHtoZWeirate, address payable _wallet,uint256 _minTransAmount,uint256 _ethCap, address tokenAddress, address payable presaleAddress,address tempMngrAddress,uint256 _goldListPeriod,uint256 _postGoldPeriod,uint256 _goldPeriodCap,uint256 _vestedMintStartTime,uint256 _calenderYearStart) public {
	...
	//zebiZCOShare=SafeMath.mul(500000000,(100000000));
	zebiZCOShare= 50000000000000000;
	crowdsaleZCOCap=zebiZCOShare;
	goldPeriodCap=_goldPeriodCap;
	//calenderYearMintCap = SafeMath.div((zebiZCOShare.mul(2)),8);
	calenderYearMintCap = zebiZCOShare * 2 / 8;
	...
	calenderYearEnd=(calenderYearStart+365 days )- 1;
  }
  ...
  function buyTokens(address payable beneficiary) public payable {
   ...
   /* if(goldListPeriodFlag){
    ...
    if((msg.value>ethCap.sub(mainWeiRaised)) && !goldListPeriodFlag){
		...
		//extraEth=(msg.value).sub(weiAmount);
        extraEth = SafeMath.sub(msg.value, weiAmount);
	 }
  ...
  function mintvestedTokens (address partnerAddress,uint256 zweitokens) external onlyOwner returns(bool){
      ...
      //year
      //uint256 currentYearCounter=SafeMath.div((SafeMath.sub(now,calenderYearStart)),365 days);
      uint256 currentYearCounter=(SafeMath.sub(now,calenderYearStart)) / 365 days;
      ...
      if(now>calenderYearEnd && currentYearCounter>=1){
          ...
          currentYearMinted=0;
          calenderYearStart=calenderYearEnd+((currentYearCounter-1)* 365 days) +1;
          calenderYearEnd=(calenderYearStart+ 365 days )- 1;
      }
  ...
  function refund() external inCancelledList1 inRefundPeriod1 {  
    require(mainCancelledList[msg.sender]);  
    ...
    uint256 refundBalance = SafeMath.add(mainContribution[msg.sender], presaleContribution) ;
  ...
  function forcedRefund(address payable _from) external onlyOwner {
    ...
    uint256 refundBalance = SafeMath.add(mainContribution[_from], presaleContribution) ;
