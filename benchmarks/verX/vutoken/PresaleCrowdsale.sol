...
contract Whitelist is Ownable {
    ...
    */
    function addManyToWhitelist(address[] calldata _beneficiaries) external onlyAuthorized {
        for (uint i = 0; i < _beneficiaries.length; i++) {
            whitelist[_beneficiaries[i]] = true;
            emit UserAllowed(_beneficiaries[i]);
        }
    }
...
contract BaseCrowdsale is AllowanceCrowdsale, PostDeliveryCrowdsale, Ownable {
    ...
    */
        ...
        TimedCrowdsale(_openingTime, _closingTime)
    {
        ...
        require(_deliveryTime < now + 8640000 seconds);
...
contract PresaleCrowdsale is BaseCrowdsale {
    ...
    */
    function getPhaseRate(uint _phase)
    public
    view
    returns (uint)
    {
        uint rate = phases[_phase].rate;
        assert(rate > 0);
        return rate;
    }
    ...
    */
    function getPhaseCap(uint _phase)
    public
    view
    returns (uint)
    {
        uint cap = phases[_phase].cap;
        assert(cap > 0);
        return cap;
    }
    ...
    */
    ...
    returns (uint)
    {
        if (tokensSold <= _getPhaseUpperLimit(uint(0))) {
            return 0;
        } else if (tokensSold <= _getPhaseUpperLimit(uint(1))) {
            return 1;
        } else {
            assert(tokensSold <= _getPhaseUpperLimit(uint(2)));
            return 2;
        }
    }
    ...
    */
    function _getPhaseUpperLimit(uint _phase)
    internal
    view
    returns (uint)
    {
        uint limit = phases[_phase].limit;
        assert(limit > 0);
        return limit;
    }
    ...
    * @return The number of tokens _weiAmount wei will buy at present time
    */
    function _calcTokenAmount(uint _phase, uint _weiAmount, uint _tokensSold)
    private
    view
    returns (uint tokensBought)
    {
        ...
        if(_phase == 0) {
            tokensBought = _weiAmount * 7500;

            if (_tokensSold.add(tokensBought) > _getPhaseUpperLimit(_phase)) {
                uint tokens = _getPhaseUpperLimit(_phase).sub(_tokensSold);

                uint nextPhase = _phase + 1;
                require(nextPhase < 3);
                tokensBought = _calcTokenAmount(nextPhase, _weiAmount.sub(tokens / 7500), _tokensSold.add(tokens));

                tokensBought = tokensBought.add(tokens);
            }
        }
        else if(_phase == 1) {
            tokensBought = _weiAmount * 6900;
            ...
            if (_tokensSold.add(tokensBought) > _getPhaseUpperLimit(_phase)) {
                ...
                uint nextPhase = _phase + 1;
                require(nextPhase < 3);
                tokensBought = _calcTokenAmount(nextPhase, _weiAmount.sub(tokens / 6900), _tokensSold.add(tokens));

                tokensBought = tokensBought.add(tokens);
            }
        }
        else if(_phase == 2) {
            tokensBought = _weiAmount * 6300;
            ...
            if (_tokensSold.add(tokensBought) > _getPhaseUpperLimit(_phase)) {
                ...
                uint nextPhase = _phase + 1;
                require(nextPhase < 3);
                tokensBought = _calcTokenAmount(nextPhase, _weiAmount.sub(tokens / 6300), _tokensSold.add(tokens));
...
contract VUToken is DetailedERC20, BurnableToken, PausableToken {
    ...
    */
    function massTransfer(address[] calldata _recipients, uint[] calldata _amounts) external returns (bool) {
...
contract PresaleDeployer {
    ...
    constructor() public {
        ...
        w = new Whitelist();
        i = new PresaleCrowdsale(v, address(w), address(0xdEaDbeEF11111111111111111111111111111111), address(0xDEadbeef22222222222222222222222222222222), 1525039200, 1530309600, 1530396000);
    }
}
